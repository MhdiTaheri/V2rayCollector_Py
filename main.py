    ...
    if os.path.exists(config_folder):
        for folder in os.listdir(config_folder):
            ...
            if os.path.isdir(folder_path):
                shutil.rmtree(folder_path)  # Remove all subdirectories

    if not os.path.exists(config_folder):
        os.makedirs(config_folder)

    for config in configs:
        ip = config.split('//')[1].split('/')[0]  # Extracting IP from config URL
        ...
        if region:
            ...
            with open(os.path.join(region_folder, 'config.txt'), 'a') as file:
                file.write(config + '\n')
